<# :
    ...
    set "AutoDelete=false"               :: Delete this file after execution
    ...
    if "%Show_Loading%"=="false" (
        if not DEFINED IS_MINIMIZED set IS_MINIMIZED=1 && start "" /min "%~f0" %args% && exit
...
;       )) > "%temp%\%~nx0" & start "" cmd.exe /%environment% "%temp%\%~nx0" %args% & exit)
...
;   REM LINES STARTING WITH ';' WILL BE IGNORED IF NOT LAUNCHED FROM 'C' AND "Ensure_Local_Running=true"
    ...
    REM PowerShell self-read, skipping batch part. Supports arguments including "" () characters
    ...
    set "PWSHerrorlevel=%errorlevel%"

    REM OPTIONNAL BATCH COMMANDS AFTER POWERSHELL
;   REM LINES STARTING WITH ';' WILL BE IGNORED IF NOT LAUNCHED FROM 'C' AND "Ensure_Local_Running=true"

    if "%AutoDelete%"=="true" ((goto) 2>nul & del "%~f0") else (exit /b %PWSHerrorlevel%)
#>
